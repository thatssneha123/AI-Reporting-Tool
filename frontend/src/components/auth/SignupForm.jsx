...
export default function SignupForm({ onSubmit }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <input className="rounded border-2 border-[var(--border)] bg-white p-2 font-semibold text-[var(--text-primary)] shadow-[2px_2px_0_rgba(17,24,39,0.9)]" placeholder="Name" value={form.name} onChange={e => setForm({...form, name: e.target.value})} />
      ...
      <button className="rounded border-2 border-[var(--border)] bg-[var(--accent)] p-2 font-bold text-white shadow-[4px_4px_0_rgba(17,24,39,0.9)]" onClick={() => onSubmit(form)}>Sign Up</button>
    </div>
  );
}
